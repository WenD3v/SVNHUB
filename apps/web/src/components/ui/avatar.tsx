"use client";

import * as AvatarPrimitive from "@radix-ui/react-avatar";
import * as React from "react";

import { getAvatarColorClass, getInitials } from "@/lib/avatar-colors";
import { cn } from "@/lib/utils";

export function Avatar({
  className,
  ...props
}: React.ComponentProps<typeof AvatarPrimitive.Root>) {
  return (
    <AvatarPrimitive.Root
      className={cn("relative flex size-8 shrink-0 overflow-hidden rounded-full", className)}
      {...props}
    />
  );
}

export function AvatarImage({
  className,
  ...props
}: React.ComponentProps<typeof AvatarPrimitive.Image>) {
  return (
    <AvatarPrimitive.Image className={cn("aspect-square size-full", className)} {...props} />
  );
}

export function AvatarFallback({
  className,
  username,
  children,
  ...props
}: React.ComponentProps<typeof AvatarPrimitive.Fallback> & { username?: string }) {
  const colorClass = username ? getAvatarColorClass(username) : "bg-muted text-muted-foreground";
  const label = children ?? (username ? getInitials(username) : "?");

  return (
    <AvatarPrimitive.Fallback
      className={cn(
        "flex size-full items-center justify-center rounded-full text-xs font-medium",
        colorClass,
        className,
      )}
      {...props}
    >
      {label}
    </AvatarPrimitive.Fallback>
  );
}
